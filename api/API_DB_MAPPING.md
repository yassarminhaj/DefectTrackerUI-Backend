...
| `GET /api/v1/defects/{defectId}/attachments/{attachmentId}/content` | `defect_attachments`, filesystem storage | Authenticated, scoped read. Streams the physical file from `FILE_STORAGE_ROOT` using `storage_key` and returns the original filename for download/preview actions. |
...
| `GET /api/v1/defects/{defectId}/inline-assets/{assetId}/content` | `defect_inline_assets`, filesystem storage | Authenticated, scoped read. Streams pasted image content from `FILE_STORAGE_ROOT` using `storage_key` so saved Steps screenshots can be restored by the editor. |
...
- UI upload endpoints use JSON with base64 `contentDataUrl` in Phase 1 because the current static UI/API layer posts `application/json`. The backend now treats metadata-only upload requests as invalid so `defect_attachments.storage_key` and `defect_inline_assets.storage_key` remain synchronized with real files on disk. Real multipart upload can be added later without changing DB tables.
...
- Add multipart upload support and richer browser preview UX if/when the UI moves away from JSON base64 uploads.
